...
describe("P1 页面金额回归（服务端组件渲染，非双人E2E）", () => {
  ...
  it("未录价格保留原投资卡片，并显示成本暂估/待估值", () => {
    const html = renderToStaticMarkup(createElement(InvestmentList, { currency: "USD", entries: [{ ...row, entry_type: "investment_buy", investment_id: id, quantity_milli: 10000 }], investments: [{ id, name: "测试标的", currency: "USD", opening_quantity_milli: 0, opening_cost_minor: 0 }], valuations: [], openCreate: () => {}, openRecord: () => {}, setPriceTarget: () => {} }));
