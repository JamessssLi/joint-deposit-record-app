...
const referencePriceMinor = z.number().int().positive().max(9_999_999_999).optional();
...
export const proposalSchema = z.discriminatedUnion("type", [
  ...
  z.object({ ...common, type: z.literal("investment_buy"), amountMinor: positiveMoney, investmentId, quantityMilli, unitPriceMinor: referencePriceMinor }).strict(),
  z.object({ ...common, type: z.literal("investment_sell"), amountMinor: z.number().int().nonnegative().max(9_999_999_999), investmentId, quantityMilli, unitPriceMinor: referencePriceMinor }).strict(),
  z.object({ ...common, type: z.literal("dividend"), amountMinor: positiveMoney, investmentId }).strict(),
  z.object({ ...common, type: z.literal("investment_valuation"), amountMinor: z.literal(0), investmentId, unitValueMinor: positiveMoney }).strict(),
]);
