...
export function InvestmentList({ investments, valuations, entries, currency, openCreate }: { investments: Row[]; valuations: Row[]; entries: Row[]; currency: string; openCreate: () => void }) { return <div className="space-y-4"><div className="flex justify-between rounded-2xl bg-[#edf7ed] p-5"><div><b>投资台账</b><p className="mt-1 text-sm text-gray-500">这里仅展示标的与持仓。交易、分红和估值请从页面顶部“新建账本记录”统一提交审批。</p></div><button onClick={openCreate} className="rounded-xl bg-[#1f5243] px-4 py-2 text-sm font-bold text-white">新增投资标的</button></div>{investments.length ? investments.map((investment) => { const id = String(investment.id); const records = entries.filter((entry) => entry.investment_id === id); let position;
    let valuation;
    let latest;
    try {
