...
type Props = { household: { id: string; name: string; reportingCurrency: string }; userId: string; role: string; entries: Row[]; proposals: Row[]; investments: Row[]; valuations: Row[]; members: Row[] };
...
export default function AppClient({ household, userId, role, entries, proposals, investments, valuations, members }: Props) {
  const router = useRouter();
  const [tab, setTab] = useState("总览"); const [recordOpen, setRecordOpen] = useState(false); const [investmentOpen, setInvestmentOpen] = useState(false); const [priceTarget, setPriceTarget] = useState<Row | null>(null); const [inviteOpen, setInviteOpen] = useState(false); const [message, setMessage] = useState("");
  ...
  return <main className="min-h-screen bg-[#f3f2ed] p-4 text-[#1d3029] md:p-7"><div className="mx-auto max-w-7xl rounded-3xl border bg-[#fbfaf6]"><header className="flex flex-wrap justify-between gap-3 border-b p-5"><div><p className="text-xs tracking-widest text-[#587064]">在线共同账本 · {role === "owner" ? "管理员" : "成员"}</p><h1 className="text-2xl font-bold">{household.name}</h1><p className="mt-1 text-xs text-gray-500">{members.length}/2 位成员 · {pending.length} 笔待审批</p></div><div className="flex flex-wrap gap-2"><button onClick={() => setRecordOpen(true)} className="rounded-xl bg-[#1f5243] px-4 py-2 text-sm font-bold text-white"><Plus size={16} className="inline"/> 新建记录</button>{role === "owner" && <button onClick={() => setInviteOpen(true)} className="rounded-xl border px-3 py-2 text-sm font-bold"><UserPlus size={16} className="mr-1 inline"/>邀请伴侣</button>}<button onClick={() => router.refresh()} className="rounded-xl border p-2" aria-label="刷新"><RefreshCcw size={17}/></button><button onClick={logout} className="rounded-xl border p-2" aria-label="退出"><LogOut size={17}/></button></div></header><nav className="flex gap-1 overflow-auto border-b p-2">{["总览", "审批中心", "流水", "投资"].map((item) => <button key={item} onClick={() => setTab(item)} className={`rounded-xl px-4 py-2 text-sm font-bold ${tab === item ? "bg-[#1f5243] text-white" : ""}`}>{item}{item === "审批中心" && pending.length > 0 ? ` (${pending.length})` : ""}</button>)}</nav><section className="p-5 md:p-7">{tab === "总览" && <Dashboard currency={household.reportingCurrency} entries={posted}/>} {tab === "审批中心" && <Approvals proposals={proposals} userId={userId} decide={decide} currency={household.reportingCurrency}/>} {tab === "流水" && <Ledger entries={posted} currency={household.reportingCurrency}/>} {tab === "投资" && <InvestmentList investments={investments} valuations={valuations} entries={posted} currency={household.reportingCurrency} openCreate={() => setInvestmentOpen(true)} openRecord={() => setRecordOpen(true)} setPriceTarget={setPriceTarget}/>}</section></div>{recordOpen && <RecordModal close={() => setRecordOpen(false)} household={household} investments={investments} refresh={() => router.refresh()} setMessage={setMessage}/>} {investmentOpen && <InvestmentCreate close={() => setInvestmentOpen(false)} household={household} refresh={() => router.refresh()} setMessage={setMessage}/>} {priceTarget && <PriceModal close={() => setPriceTarget(null)} investment={priceTarget} refresh={() => router.refresh()} setMessage={setMessage}/>} {inviteOpen && <InviteModal close={() => setInviteOpen(false)} householdId={household.id} setMessage={setMessage}/>} {message && <div className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 rounded-xl bg-[#193d32] px-5 py-3 text-sm text-white">{message}</div>}</main>;
}
export function Dashboard({ currency, entries }: { currency: string; entries: Row[] }) {
  ...
  return <div className="space-y-6"><div className="grid gap-4 md:grid-cols-3"><Card label="共同现金" value={cash.amountMinor === null ? "待完善汇率" : money(cash.amountMinor, currency)}/><Card label="已入账记录" value={`${entries.length} 笔`}/><Card label="报告币种" value={currency}/></div><div className="rounded-2xl border bg-white p-4 text-sm"><b>原币现金</b><div className="mt-2 flex flex-wrap gap-4">{Object.entries(cashByCurrency).map(([code, amount]) => <span key={code}>{code} {money(amount, code)}</span>)}</div>{cash.missingCurrencies.length > 0 && <p className="mt-2 text-amber-800">缺少 {cash.missingCurrencies.join(" / ")} 已确认汇率，暂不显示折算合计。</p>}</div><section className="rounded-2xl border bg-white p-5"><b>消费趋势</b>{missingTrend ? <Empty text="消费包含未配置汇率的币种，折算趋势待完善。原币金额可在流水查看。"/> : trend.length === 0 ? <Empty text="暂无消费记录。"/> : <div className="mt-4 h-72"><ResponsiveContainer><LineChart data={trend.map((point) => ({ date: point.date, total: point.amountMinor! / 100 }))}><XAxis dataKey="date" tick={{ fontSize: 10 }}/><YAxis tick={{ fontSize: 10 }}/><Tooltip formatter={(value: number) => money(Math.round(value * 100), currency)}/><Line type="monotone" dataKey="total" stroke="#1f5243" strokeWidth={3}/></LineChart></ResponsiveContainer></div>}</section></div>;
}
function Approvals({ proposals, userId, decide, currency }: { proposals: Row[]; userId: string; decide: (id: string, approve: boolean) => void; currency: string }) { return <div className="space-y-3">{proposals.length ? proposals.map((proposal) => { const payload = proposal.payload as Record<string, unknown>; const mine = proposal.submitter_id === userId; return <article key={String(proposal.id)} className="rounded-2xl border bg-white p-5"><div className="flex justify-between gap-3"><div><b>{String(payload.title)}</b><p className="mt-1 text-xs text-gray-500">{String(payload.type)} · {Number(payload.amountMinor) ? money(Number(payload.amountMinor), String(payload.currency || currency)) : "非现金操作"}</p></div><span className="rounded-full bg-amber-100 px-2 py-1 text-xs">{String(proposal.status)}</span></div>{["pending_approval", "overdue_pending"].includes(String(proposal.status)) && <div className="mt-4">{mine ? <span className="text-xs text-gray-500">你不能审批自己提交的记录。</span> : <><button onClick={() => decide(String(proposal.id), false)} className="mr-2 rounded-lg border px-3 py-2 text-sm">驳回</button><button onClick={() => decide(String(proposal.id), true)} className="rounded-lg bg-[#1f5243] px-3 py-2 text-sm text-white">批准</button></>}</div>}</article>; }) : <Empty text="暂无审批记录。"/>}</div>; }
function Ledger({ entries, currency }: { entries: Row[]; currency: string }) { return <div className="overflow-hidden rounded-2xl border bg-white">{entries.length ? entries.map((entry) => <div key={String(entry.id)} className="flex justify-between gap-3 border-b p-4"><div><b>{String(entry.title)}</b><p className="mt-1 text-xs text-gray-500">{String(entry.occurred_at)} · {String(entry.entry_type)} · {String(entry.category || "")}</p></div><b>{money(Number(entry.amount_minor), String(entry.currency || currency))}</b></div>) : <Empty text="暂无已入账流水。"/>}</div>; }
export function InvestmentList({ investments, valuations, entries, currency, openCreate, openRecord, setPriceTarget }: { investments: Row[]; valuations: Row[]; entries: Row[]; currency: string; openCreate: () => void; openRecord: () => void; setPriceTarget: (investment: Row) => void }) { return <div className="space-y-4"><div className="flex justify-between rounded-2xl bg-[#edf7ed] p-5"><div><b>投资台账</b><p className="mt-1 text-sm text-gray-500">标的直接创建；交易和分红需对方审批。价格由成员手工维护。</p></div><button onClick={openCreate} className="rounded-xl bg-[#1f5243] px-4 py-2 text-sm font-bold text-white">新增投资标的</button></div>{investments.length ? investments.map((investment) => { const id = String(investment.id); const records = entries.filter((entry) => entry.investment_id === id); let position;
    let valuation;
    let latest;
    try {
    ...
    const market = valuation.marketMinor; return <article key={id} className="rounded-2xl border bg-white p-5"><div className="flex flex-wrap justify-between gap-3"><div><h2 className="font-bold">{String(investment.name)}</h2><p className="mt-1 text-xs text-gray-500">{String(investment.currency)} · 可卖 {position.quantityMilli / 1000} 份</p></div><button onClick={openRecord} className="rounded-xl border px-3 py-2 text-sm">提交交易</button><button onClick={() => setPriceTarget(investment)} className="rounded-xl border px-3 py-2 text-sm">更新最新价格</button></div><div className="mt-5 grid gap-3 md:grid-cols-5"><Card label={valuation.source === "cost_estimate" ? "当前市值（成本暂估）" : "当前市值"} value={money(market, String(investment.currency))}/><Card label="剩余成本" value={money(position.remainingCostMinor, String(investment.currency))}/><Card label="未实现收益" value={valuation.unrealizedGainMinor === null ? "待估值" : money(valuation.unrealizedGainMinor, String(investment.currency))}/><Card label="已实现收益" value={money(position.realizedGainMinor, String(investment.currency))}/><Card label="累计分红" value={money(position.dividendMinor, String(investment.currency))}/></div><div className="mt-5 border-t pt-3 text-sm"><p className="mb-3 text-xs text-gray-500">{latest ? `人工估值日期：${latest.valueDate}` : "尚无人工估值；暂按剩余成本展示。"}</p><b>交易历史</b>{records.length ? records.map((entry) => <p key={String(entry.id)} className="mt-2">{String(entry.occurred_at)} · {String(entry.entry_type)} · {money(Number(entry.amount_minor), String(entry.currency || currency))}</p>) : <p className="mt-2 text-gray-500">暂无已批准交易。</p>}</div></article>; }) : <Empty text="尚未有投资标的。请先创建零持仓标的，再提交交易审批。"/>}</div>; }
export function RecordModal({ close, household, investments, refresh, setMessage, readOnly = false }: { close: () => void; household: Props["household"]; investments: Row[]; refresh: () => void; setMessage: (value: string) => void; readOnly?: boolean }) {
