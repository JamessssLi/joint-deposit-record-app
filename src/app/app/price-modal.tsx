...
export default function PriceModal({ close, investment, refresh, setMessage }: { close: () => void; investment: Row; refresh: () => void; setMessage: (value: string) => void }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const priceTenThousandths = parseFixedDecimal(price, 4, { label: "单位价格" });
      const response = await fetch(`/api/investments/${String(investment.id)}/price`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ valueDate: new Date().toISOString().slice(0, 10), priceTenThousandths, note: note || undefined }) });
      const body = await response.json().catch(() => ({}));
      if (!response.ok) return setMessage(body.error || "价格更新失败");
      close(); setMessage("最新价格已直接更新，并已保留价格历史。"); refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "价格更新失败");
    }
  }
